...
namespace App\Http\Controllers\API\Aduan;
...
class AduanController extends ApiController
{
    ...
    public function store(StoreAduanRequest $request)
    {
        try {
            $user = $request->user();
            $aduan = $this->aduanService->createAduan($user->id, $user->role, $request->validated());
