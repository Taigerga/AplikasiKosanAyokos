                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center justify-between mb-3">
                                                ...
                                                <span class="text-xs px-2 py-1 rounded-full 
                                                    {{ $kontrak->statusWarna == 'green' ? 'bg-emerald-50 text-emerald-600' :
                            ($kontrak->statusWarna == 'yellow' ? 'bg-yellow-50 text-yellow-600' :
                                ($kontrak->statusWarna == 'red' ? 'bg-red-50 text-red-600' :
                                    'bg-gray-100 text-slate-500')) }}">
                                            ...
                                            <p class="text-sm text-slate-200 mb-3">Kamar {{ $kontrak->kamar->nomor_kamar }}</p>
                                ...
                                <div
                                    ...
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 rounded-full 
                                            {{ $pembayaran->status_pembayaran == 'lunas' ? 'bg-indigo-50' :
                                ...
                                ($pembayaran->status_pembayaran == 'terlambat' ? 'bg-red-50' :
                                    'bg-gray-100')) }} flex items-center justify-center">
                                            <i class="fas fa-{{ $pembayaran->status_pembayaran == 'lunas' ? 'check' : 'clock' }} 
                                                {{ $pembayaran->status_pembayaran == 'lunas' ? 'text-indigo-600' :
                                ...
                                ($pembayaran->status_pembayaran == 'terlambat' ? 'text-red-600' :
                                    'text-slate-500')) }}"></i>
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-400">{{ $pembayaran->bulan_tahun }}</p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-bold text-white">Rp {{ number_format($pembayaran->jumlah, 0, ',', '.') }}</p>
                                        <span class="inline-block px-2 py-1 text-xs rounded-full 
                                            {{ $pembayaran->status_pembayaran == 'lunas' ? 'bg-indigo-50 text-white-600' :
                    ...
                    <div class="space-y-4">
                        @foreach($kontrakBerakhirSegera->take(3) as $kontrak)
                            <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <h3 class="font-semibold text-slate-800 mb-2">{{ $kontrak->kos->nama_kos }}</h3>
                                        <p class="text-sm text-slate-500 mb-2">Kamar {{ $kontrak->kamar->nomor_kamar }}</p>
                    ...
                    <div class="bg-slate-900/50 backdrop-blur-sm border border-sky-900/50 rounded-xl p-4">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="px-3 py-1 rounded-full text-sm font-medium
                                {{ $user->status_penghuni == 'aktif' ? 'bg-emerald-50 text-emerald-600' :
