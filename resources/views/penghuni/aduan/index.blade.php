...
@section('title', 'Aduan Saya - Penghuni AyoKos')
...
<div class="p-4 md:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
    ...
    <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between">
            ...
            <a href="{{ route('penghuni.aduan.create') }}" class="mt-4 md:mt-0 px-6 py-3 bg-lime-400 hover:bg-lime-500 text-black font-black border-2 border-black shadow-[2px_2px_0px_#000] hover:shadow-[3px_3px_0px_#000] transition-all uppercase tracking-wide">
    ...
    <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] overflow-hidden">
        <div class="p-6 border-b-2 border-gray-200">
            <form method="GET" action="{{ route('penghuni.aduan.index') }}">
                <div class="flex flex-col md:flex-row gap-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request('status'))
                            <a href="{{ route('penghuni.aduan.index') }}" class="px-6 py-3 bg-gray-100 text-gray-700 font-black border-2 border-black shadow-[2px_2px_0px_#000] hover:shadow-[3px_3px_0px_#000] hover:translate-y-[-1px] transition-all uppercase tracking-wide">
                    ...
                    <tbody class="divide-y-2 divide-gray-200">
                        @foreach($aduans as $index => $aduan)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-4 py-4 text-sm font-bold text-gray-700">{{ $aduans->firstItem() + $index }}</td>
                                ...
                                <td class="px-4 py-4">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-black border-2 border-black {{ $color }} text-black">
                                        {{ ucfirst(str_replace('_', ' ', $aduan->status_aduan)) }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-sm font-bold text-gray-600">{{ \Carbon\Carbon::parse($aduan->created_at)->format('d M Y') }}</td>
                                <td class="px-4 py-4 text-center">
                                    <a href="{{ route('penghuni.aduan.show', $aduan->id_aduan) }}" class="inline-flex items-center px-3 py-2 bg-sky-400 hover:bg-sky-500 text-black font-black text-sm border-2 border-black shadow-[2px_2px_0px_#000] hover:shadow-[3px_3px_0px_#000] transition-all">
        ...
        @else
            <div class="text-center py-12">
                ...
                <a href="{{ route('penghuni.aduan.create') }}" class="inline-flex items-center px-6 py-3 bg-lime-400 hover:bg-lime-500 text-black font-black border-2 border-black shadow-[2px_2px_0px_#000] hover:shadow-[3px_3px_0px_#000] transition-all uppercase tracking-wide">
                    <i class="fas fa-plus mr-2"></i>
                    Buat Aduan Baru
                </a>
            </div>
