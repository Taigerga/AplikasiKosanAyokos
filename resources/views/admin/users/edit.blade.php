...
@section('title', 'Edit User - AyoKos')
...
<div class="p-4 md:p-6 lg:p-8 space-y-6 max-w-2xl mx-auto">
    ...
    <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-6">
        <h2 class="text-xl font-black mb-6"><i class="fas fa-user-edit mr-3 text-sky-500"></i>Edit Admin: {{ $user->username }}</h2>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                    ...
                    @php $nama = $user->admin->nama ?? ''; @endphp
                    <input type="text" name="nama" value="{{ old('nama', $nama) }}" class="form-input @error('nama') is-invalid @enderror" required>
                    @error('nama')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label class="form-label"><i class="fas fa-envelope mr-2 text-sky-400"></i>Email</label>
                    @php $email = $user->admin->email ?? ''; @endphp
                    <input type="email" name="email" value="{{ old('email', $email) }}" class="form-input @error('email') is-invalid @enderror" required>
                    @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label class="form-label"><i class="fas fa-phone mr-2 text-sky-400"></i>No. HP</label>
                    @php $no_hp = $user->admin->no_hp ?? ''; @endphp
                    <input type="text" name="no_hp" value="{{ old('no_hp', $no_hp) }}" class="form-input @error('no_hp') is-invalid @enderror" required>
