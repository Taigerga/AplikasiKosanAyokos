...
@section('title', 'Kelola User - Admin AyoKos')
        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                <li class="inline-flex items-center">
                    <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center text-sm font-bold text-gray-600 hover:text-black transition-colors">
    ...
    <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div class="flex gap-3">
                <a href="{{ route('admin.users.create') }}" class="bg-emerald-400 border-2 border-black shadow-[2px_2px_0px_#000] hover:shadow-[4px_4px_0px_#000] hover:-translate-y-0.5 transition-all px-4 py-2 font-bold text-sm flex items-center gap-2">
        ...
        <form method="GET" class="flex flex-col md:flex-row gap-3 mb-6">
            <input type="text" name="search" placeholder="Cari username..." value="{{ request('search') }}" class="border-2 border-black px-3 py-2 font-bold text-sm flex-1">
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr class="hover:bg-yellow-50">
                            <td class="border-2 border-black px-4 py-3 text-sm font-bold">{{ $user->id }}</td>
                            <td class="border-2 border-black px-4 py-3 text-sm font-bold">{{ $user->username }}</td>
