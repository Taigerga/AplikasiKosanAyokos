...
@section('title', 'Data Pemilik - Admin AyoKos')
...
<div class="p-6">
    ...
    <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-6">
        <form method="GET" action="{{ route('admin.data-pemilik.index') }}" class="flex flex-wrap gap-4 mb-6">
                ...
                <tbody>
                    @forelse($dataPemilik as $i => $pemilik)
                        <tr class="border-b-2 border-black hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 font-bold">{{ $pemilik->user->name ?? $pemilik->name ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">{{ $pemilik->user->email ?? $pemilik->email ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">{{ $pemilik->no_hp ?? '-' }}</td>
                            <td class="px-4 py-3">
                                @php
                                    $statusColor = [
                                        'aktif' => 'bg-green-300',
                            ...
                            <td class="px-4 py-3 font-bold">{{ ($pemilik->created_at ?? now())->format('d M Y') }}</td>
                            <td class="px-4 py-3">
                                <a href="{{ route('admin.data-pemilik.show', $pemilik->id_pemilik) }}" class="bg-black text-white font-black px-3 py-1 text-sm border-2 border-black shadow-[3px_3px_0px_#000] hover:shadow-[1px_1px_0px_#000] transition-all inline-block">
