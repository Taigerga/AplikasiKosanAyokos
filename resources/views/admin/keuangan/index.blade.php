...
@section('title', 'Keuangan Platform - Admin AyoKos')
...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <div class="bg-green-300 border-4 border-black p-4 shadow-[4px_4px_0px_#000]">
            ...
            <p class="text-2xl font-black mt-1">Rp{{ number_format($ringkasan['totalPendapatanPlatform'] ?? 0, 0, ',', '.') }}</p>
        </div>
        <div class="bg-blue-300 border-4 border-black p-4 shadow-[4px_4px_0px_#000]">
            ...
            <p class="text-2xl font-black mt-1">Rp{{ number_format($ringkasan['totalPendapatanTahun'] ?? 0, 0, ',', '.') }}</p>
        </div>
        <div class="bg-yellow-300 border-4 border-black p-4 shadow-[4px_4px_0px_#000]">
            ...
            <p class="text-2xl font-black mt-1">Rp{{ number_format($ringkasan['totalPendapatanBulan'] ?? 0, 0, ',', '.') }}</p>
        </div>
        <div class="bg-purple-300 border-4 border-black p-4 shadow-[4px_4px_0px_#000]">
            ...
            <p class="text-2xl font-black mt-1">{{ $ringkasan['totalTransaksiLunas'] ?? 0 }}</p>
        </div>
        <div class="bg-orange-300 border-4 border-black p-4 shadow-[4px_4px_0px_#000]">
            ...
            <p class="text-2xl font-black mt-1">{{ $ringkasan['totalTransaksiTahun'] ?? 0 }}</p>
        </div>
    </div>
                ...
                <tbody>
                    @forelse($pendapatanBulanan as $item)
                        <tr class="border-b-2 border-black hover:bg-gray-50">
                            <td class="px-4 py-3 font-bold">{{ $item->bulan_nama ?? $item['bulan_nama'] ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">Rp{{ number_format($item->pendapatan_pemilik ?? $item['pendapatan_pemilik'] ?? 0, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 font-bold text-green-700">Rp{{ number_format($item->pendapatan_platform ?? $item['pendapatan_platform'] ?? 0, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 font-bold">{{ $item->jumlah_transaksi ?? $item['jumlah_transaksi'] ?? 0 }}</td>
                        </tr>
                    @empty
                        <tr>
            ...
            <table class="w-full border-collapse">
                ...
                <tbody>
                    @forelse($transaksiTerbaru as $t)
                        <tr class="border-b-2 border-black hover:bg-gray-50">
                            <td class="px-4 py-3 font-bold">{{ $t->kos->nama_kos ?? $t['nama_kos'] ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">{{ $t->penghuni->user->name ?? $t['nama_penghuni'] ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">{{ ($t->created_at ?? $t['tanggal'] ?? now())->format('d M Y') }}</td>
                            <td class="px-4 py-3 font-bold">Rp{{ number_format($t->total_bayar ?? $t['total_bayar'] ?? 0, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 font-bold">Rp{{ number_format($t->bagian_pemilik ?? $t['bagian_pemilik'] ?? 0, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 font-bold text-green-700">Rp{{ number_format($t->bagian_platform ?? $t['bagian_platform'] ?? 0, 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            @php $totalPemilikBerbayar = count($statistikPemilik); @endphp
            <span class="bg-green-300 border-2 border-black px-3 py-1 text-sm font-black">Total Pemilik Berbayar: {{ $totalPemilikBerbayar }}</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full border-collapse">
                <thead>
                ...
                <tbody>
                    @forelse($statistikPemilik as $sp)
                        <tr class="border-b-2 border-black hover:bg-gray-50">
                            <td class="px-4 py-3 font-bold">{{ $sp->nama_kos ?? $sp['nama_kos'] ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">Rp{{ number_format($sp->pendapatan_pemilik ?? $sp['pendapatan_pemilik'] ?? 0, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 font-bold text-green-700">Rp{{ number_format($sp->pendapatan_platform ?? $sp['pendapatan_platform'] ?? 0, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 font-bold">{{ $sp->jumlah_transaksi ?? $sp['jumlah_transaksi'] ?? 0 }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center font-bold text-gray-500">Belum ada data</td>
