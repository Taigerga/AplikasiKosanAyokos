...
@section('title', 'Data Penghuni - Admin AyoKos')
...
<div class="p-6">
    ...
    <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-6">
        <form method="GET" action="{{ route('admin.data-penghuni.index') }}" class="flex flex-wrap gap-4 mb-6">
            <div class="flex-1 min-w-[200px]">
                <label class="block text-sm font-black mb-1">Cari</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama / Email / No HP" class="w-full border-2 border-black px-3 py-2 font-bold focus:outline-none focus:ring-0">
            </div>
            <div>
                ...
                <select name="status" class="border-2 border-black px-3 py-2 font-bold bg-white focus:outline-none focus:ring-0">
                    ...
                    <option value="dibatasi" {{ request('status') == 'dibatasi' ? 'selected' : '' }}>Dibatasi</option>
        ...
        <div class="overflow-x-auto">
            <table class="w-full border-collapse">
                ...
                <tbody>
                    @forelse($dataPenghuni as $i => $penghuni)
                        <tr class="border-b-2 border-black hover:bg-gray-50">
                            <td class="px-4 py-3 font-bold">{{ $loop->iteration }}</td>
                            <td class="px-4 py-3 font-bold">{{ $penghuni->user->name ?? $penghuni->name ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">{{ $penghuni->user->email ?? $penghuni->email ?? '-' }}</td>
                            <td class="px-4 py-3 font-bold">{{ $penghuni->no_hp ?? '-' }}</td>
                            <td class="px-4 py-3">
                                @php
                                    $statusColor = [
                                        'aktif' => 'bg-green-300',
                            ...
                            <td class="px-4 py-3 font-bold">{{ ($penghuni->created_at ?? now())->format('d M Y') }}</td>
                            <td class="px-4 py-3">
                                <a href="{{ route('admin.data-penghuni.show', $penghuni->id_penghuni) }}" class="bg-black text-white font-black px-3 py-1 text-sm border-2 border-black shadow-[3px_3px_0px_#000] hover:shadow-[1px_1px_0px_#000] transition-all inline-block">
