...
@section('title', 'Analisis - Admin AyoKos')
        ...
        <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-5">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-black">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-5">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-black">{{ $totalKos }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-5">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-black">{{ $totalAduanTerbuka }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white border-4 border-black shadow-[4px_4px_0px_#000] p-5">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-black">{{ $totalKontrakAktif }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-gray-100 border-2 border-black p-4">
                ...
                <p class="text-sm text-gray-600">
                    <span class="font-black text-black">{{ $pemilikTop->nama ?? '-' }}</span>
                    dengan pendapatan
                    <span class="font-black text-black">Rp {{ number_format($pemilikTop->total_pendapatan ?? 0, 0, ',', '.') }}</span>
                </p>
            </div>
            <div class="bg-gray-100 border-2 border-black p-4">
                <div class="flex items-center mb-3">
                ...
                <p class="text-sm text-gray-600">
                    ...
                    <span class="font-black text-black">Rp {{ number_format($pendapatanRata, 0, ',', '.') }}</span>
                </p>
            </div>
            <div class="bg-gray-100 border-2 border-black p-4">
                <div class="flex items-center mb-3">
                ...
                <p class="text-sm text-gray-600">
                    <span class="font-black text-black">{{ $totalUser }}</span> pengguna terdaftar
                    ({{ $sebaranRole['admin'] }} admin, {{ $sebaranRole['pemilik'] }} pemilik, {{ $sebaranRole['penghuni'] }} penghuni)
                </p>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    if (typeof window.initAdminCharts === 'function') {
        window.initAdminCharts({
            ...
            statusKos: @json($statusKos),
            aduanPerStatus: @json($aduanPerStatus),
            userGrowth: @json($userGrowth),
            sebaranRole: @json($sebaranRole),
            topPemilik: @json($topPemilik),
        });
    }
});
</script>
